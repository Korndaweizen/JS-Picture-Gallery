...
if save_as(1) == 1
    disp([path2image fileName '.eps'])
    saveas(handle, [path2image fileName '.eps'],'epsc');
    %print('-depsc','-tiff','-r600',[path2image fileName '.eps'])
end

if save_as(3) == 1
    if latexFigureCount == 1
        save2pdf([path2image fileName], handle, 600, 16, 1);
    elseif latexFigureCount == 2
        save2pdf([path2image fileName], handle, 600, 16, 2);
    elseif latexFigureCount == 3
        save2pdf([path2image fileName], handle, 600, 14, 2);
    elseif latexFigureCount > 3
